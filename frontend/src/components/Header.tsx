...
export default function Header({ onToggleTheme, darkMode: darkMode} : HeaderProps){
    ...
    return (
        ...
        <header className={`flex flex-row w-full justify-between h-12 px-2 ${darkMode ? `bg-purple-800` : `bg-purple-400`}`}>
            ...
            {/* Middle */}
            <nav className="flex flex-row items-center text-zinc-900">
                <div className="w-18 text-center"><a href="#games" onClick={() => scrollToSpot(600)} className={`inline-block mx-2 cursor-pointer font-bold text-lg transition-all  duration-300 hover:scale-125 ${darkMode ? `text-white hover:text-zinc-900` : `text-zinc-900 hover:text-white`}`}>Games</a></div>
                <div className="w-18 text-center"><a href="#team" onClick={() => scrollToSpot(950)} className={`inline-block mx-2 cursor-pointer font-bold text-lg transition-all duration-300 hover:scale-125 ${darkMode ? `text-white hover:text-zinc-900` : `text-zinc-900 hover:text-white`}`}>Team</a></div>
                <div className="w-18 text-center"><a href="#jobs" onClick={openJobsPage} className={`inline-block mx-2 cursor-pointer font-bold text-lg transition-all duration-300 hover:scale-125 ${darkMode ? `text-white hover:text-zinc-900` : `text-zinc-900 hover:text-white`}`}>Jobs</a></div>
